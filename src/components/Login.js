  ...
  return (
    <div className="login">
      ...
      <form onSubmit={handleSubmit}>
        <label
        className={ `${focusedId ? 'focused' : ''} ${idError ? 'error' : ''}`}
        >
          ...
          onBlur={(event)=>{
            if(event.target.value === ''){setFocusedId(false);
            setIdError(true);}
          }}/>
        </label>
        {
          idError && <p className="id-error">ⓧ 정확한 이메일 주소나 전화번호를 입력하세요.</p>
        }
        {
          !isCode && (<label className={ `${focusedPw ? 'focused' : ''} ${pwError ? 'error' : ''}`}>
            <span >비밀번호</span>
            <input type="password"
            onFocus={()=>{
              setFocusedPw(true);
        ...
        {
          pwError && <p className="id-error">ⓧ 비밀번호는 4~60자 사이여야 합니다.</p>
        }
        {
      ...
      <div className="login-box">
        <label>
          <input type="checkbox"/><span>로그인 정보 저장</span>
        </label>
